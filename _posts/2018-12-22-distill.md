...
In particular, PTQ (Post-Training Quantization) and QAT (Quantization-Aware Training) require very different model scales, yet to date no study has quantitatively defined the optimal sizes for these two approaches. In this work, we conduct a thorough survey of the leading 1-bit training methods published since 2023 and estimated the optimal model size for PTQ and QAT by defining the scaling factor, providing concrete guidelines for balancing target performance and resource constraints when designing and deploying 1-bit LLMs.

## Related work about quantization method (1-bit or sub 1-bit)

### BitNet (2023) <a href="https://arxiv.org/abs/2310.11453">[3]</a>
...
$$
Scaling\ Factor = \frac{Model\ size\ required\ to\ satisfy\ FP16\ baseline\ performance}{Model\ size\ for\ FP16\ baseline}
$$
<br>
**Example**: 1bit quantization model requires 1.8× the size of the original model to perform 90% → Scaling Factor = 1.8
...
- On the other hand, the BitNet_v2 series took and used the values compared to the FP16 baseline provided by the <a href="https://arxiv.org/abs/2402.17764" style="color:blue;">[4]</a> as it was.
...
- **Sub 1-bit training**
The Sub 1-bit method fell short of the baseline on almost all benchmarks.(especially 4-bit activation)
- **1-bit training**
Lowering the weight to 1 bit in the QAT scheme did not achieve a reference point on most benchmarks, even if the activation bit was high.
- **1.58-bit training**
The PTQ method requires a model size that is two to five times higher than the QAT.
