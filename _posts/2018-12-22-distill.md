...
- To overcome the instability of Hessian-based importance scores,  
  STBLLM proposes a lightweight importance metric that considers both weight magnitude and input feature norm.
...
- Each layer’s relative importance \(\alpha_i\) is computed and used to apply  
  **aggressive compression for less critical layers**, and **preserve more precision in key layers**.
...
- Weights are divided into three zones based on their importance:
  **sparse**, **intermediate**, and **dense**.  
- Different binarization strategies are applied to each zone to reduce total bit usage  
  while maintaining task-level performance.

> 💡 **STBLLM** achieves sub-1bit quantization (as low as **0.55 bits**) on models like **LLaMA, OPT, Mistral**,  
> with **up to 20× lower perplexity** compared to existing 1-bit baselines.
